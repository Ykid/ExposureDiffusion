...
from models.ELD_model import IlluminanceCorrect, tensor2im


class ELDModelIterV2(ELDModelIter):
    def __init__(self):
        self.epoch = 0
        self.iterations = 0
    ...
    def set_input(self, data, mode='train'):
        """
        Accept both legacy SID-style batches (input/target/ratio/K) and the new
        ...
        if isinstance(data, dict) and 'X_t_blur' in data and 'Y' in data:
            ...
            if ratio is None and lambda_ref is not None and lambda_T is not None:
                ratio = float(lambda_ref) / max(float(lambda_T), 1e-8)
            self.ratio = ratio
            self.K = noise_params_T.get("K", None)
            ...
            if len(self.gpu_ids) > 0:
                ...
                if self.K is not None and not torch.is_tensor(self.K):
                    self.K = torch.tensor(self.K, device=self.gpu_ids[0])
            ...
            self.iso = data.get('ISO', None)

            # Optional bookkeeping fields so downstream logging still works.
            self.data_name = data.get('fn', None)
            self.rawpath = data.get('rawpath', None)
            self.cfa = data.get('cfa', 'bayer')
            self.aligned = True
        else:
            ...
            self.iso = data.get('ISO', None) if isinstance(data, dict) else None
    ...
    def forward(self, iter_num=0):
        # Multi-step exposure path as in the original iterative model.
        input_i = self.input
        ...
        def _forward(input_i, ratio_current):
            net_input = input_i
            metadata = None
        ...
        for i in range(iter_steps):
            if isinstance(ratio_list, torch.Tensor):
                ...
                ratio_current, ratio_next = ratio_list[i], ratio_list[i+1]
            output_list = list(_forward(input_i, ratio_current))
            output_list[0] = self.corrector(output_list[0], self.target) if self.target is not None else output_list[0]
        ...
        self.output = self.output_list[-1][0]
