"""Create lmdb dataset with exposure-aware metadata."""
import sys
...
# Ensure repo root on path for dataset imports when run as a script
ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
from dataset.blur_utils import lambda_range_from_exposure
        ...
        for i, fn in enumerate(fns):
            ...
            for j in range(N):                
                meta_info[k] = {
                    ...
                    "ccm": ccm,
                    "ISO": iso,
                    "ratio": ratios[i] if ratios is not None else 0,
                    "exposure": expo_t,
                    "lambda_ref": expo_t,
                    "lambda_T_range": lambda_range_from_exposure(expo_t),
                }                
                data = X[j].tobytes()
                str_id = '{:08}'.format(k)
                k += 1
...
def create_sony_dataset(num_samples=None): # synthetic data
    ...
    create_lmdb_train(
        fns, join(destdir, 'SID_Sony_Raw_Meta'),
        ksize=(4, 512, 512),
        stride=(4, 512, 512),
        srgb=False, 
    )
